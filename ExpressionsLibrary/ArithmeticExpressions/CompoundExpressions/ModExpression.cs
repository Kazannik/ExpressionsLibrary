...
namespace ExpressionsLibrary.ArithmeticExpressions.CompoundExpressions
{
    ...
    /// </summary>
    class ModExpression : Expression, IDecimalExpression
    {
        ...
        {
            if (RightExpression.Value == 0 && !RightExpression.IsError)
                return LeftExpression.Formula(format: format) + ArithmeticExpression.SymbolSpace + ArithmeticExpression.SymbolMod + ArithmeticExpression.SymbolSpace + ArithmeticExpression.SymbolStartError + RightExpression.Formula(format: format) + ArithmeticExpression.SymbolEndError;
            else
                return LeftExpression.Formula(format: format) + ArithmeticExpression.SymbolSpace + ArithmeticExpression.SymbolMod + ArithmeticExpression.SymbolSpace + RightExpression.Formula(format: format);
        }
